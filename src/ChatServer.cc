...
const int MAX_CONNECTIONS = 16000;//AeroChat接收的最大连接数
const int SOCKET_SNDBUF = 262144;//socket发送缓冲区的大小
const int SOCKET_RCVBUF = 262144;//socket接收缓冲区的大小
const int DB_POOL_SIZE = 100;  //MySQL连接池的大小  根据内存调整 
...
//为什么使用原子变量？  原子操作在x86上会翻译成lock前缀的指令,直接操作缓存行,比加锁快很多,这里是为了性能考虑选择原子变量
//记录连接数

ChatServer::ChatServer(EventLoop* loop, uint16_t port, int numSubReactors)
    : loop_(loop), port_(port),
      threadPool_(new EventLoopThreadPool(loop, numSubReactors > 0 ? numSubReactors : SUB_REACTOR_NUM)),
      listenFd_(-1), acceptChannel_(nullptr), sock_(), epollTimeout_(100),
      timerFd_(-1) {
      //1、设置信号处理(忽略SIGPIPE,处理SIGSEGV)
    signal(SIGPIPE, SIG_IGN);
    signal(SIGABRT, SIG_IGN);
    signal(SIGSEGV, [](int) { LOG_ERROR("SEGV caught"); exit(1); });
	...
	//此项目中这里最大连接数是1.6W/(线程数+100) +100是一个安全缓冲,防止因分配不均导致实际连接数
	//超过均值
    }
    GroupManager::instance().initDefaultGroup();//初始化群组默认123
    GroupManager::instance().loadAllGroupsFromDB();
    LOG_INFO("ChatServer初始化完成");
}

ChatServer::~ChatServer() {
    if (listenFd_ >= 0) close(listenFd_);//关闭监听 socket和timerfd,释放资源。
...
//1、启动线程池
    ...
    if (loops.size() + 1 > perLoopUsers_.size()) {
        ...
        for (size_t i = 0; i < perLoopUsers_.size(); ++i) {
            if (!perLoopUsers_[i]) {
                perLoopUsers_[i] = std::make_unique<PerLoopUsers>();
            } //为每个线程的 map 预分配内存,避免运行时rehash。每个线程平均处理 MAX_CONNECTIONS / totalThreads 个连接,
              //加 100 作为安全缓冲。
...
//3、创立监听Channel监听socket(由socket()、bind()、listen() 共用)和 accept()返回的连接socket,
//是两个不同的fd,而这里创建的acceptChannel_是对监听socket的封装。
    acceptChannel_ = std::make_unique<Channel>(loop_, listenFd_);//1、创建Channel对象,封装listenFd_,并将其与主EventLoop绑定
    acceptChannel_->setReadCallback([this] { handleAccept(); });//2、设置读回调函数,当listenFd_有可读事件(新连接到来)时Channel就会调用
...
//处理该连接的Rector的EventLoop指针,  所有该连接的IO事件和业务逻辑都将在此EventLoop执行
    //设置 socket 选项来优化网络性能
    (void)clientIp; (void)clientPort;   // 添加这一行
    ...
    if (idx < 0 || static_cast<size_t>(idx) >= perLoopUsers_.size() || !perLoopUsers_[idx]) {
    ...
    user->setMessageCallback([](int fd, const std::string& msg) {
        ...
        if (u) MessageRouter::instance().onMessage(fd, msg, u);
    });
    ...
    std::string msg = "Connected to chat server (fd:" + std::to_string(connFd) + ")";
...
void ChatServer::removeUser(int fd) {
    ...
    targetLoop->runInLoop([this, targetIdx, fd] {
        if (!perLoopUsers_[targetIdx]) return;
...
void ChatServer::redisSubLoop() {
    RedisClient::instance().subscribe("online_updates", [this](const std::string& msg) {
        broadcastOnlineUpdate(msg);
    });
}
