...
- **连接管理优化**：统一使用 ConnectionManager 管理所有连接，shared_mutex 读写锁保护，广播时按 EventLoop 分组投递，避免全局锁。
...
- **心跳保活**：客户端定时发送 ping，服务端回复 pong，空闲连接超过 180 秒自动断开
...
## ⚙️性能优化
多线程模型：主 Reactor + 子 Reactor 线程池，默认线程数 = CPU 核心数 × 4。

无锁队列：跨线程任务投递使用 ConcurrentQueue，避免锁竞争。

边缘触发 + 非阻塞 I/O：配合 epoll ET 模式，一次读取所有数据，减少系统调用。

连接管理：统一 ConnectionManager 管理所有连接，shared_mutex 读写锁，广播按 EventLoop 分组投递。

写缓冲区背压：单连接发送缓冲区上限 4MB，防止内存爆炸。

MySQL 连接池：预创建 128 个连接，复用 TCP 连接，减少握手开销。

Redis 连接池：预创建 64 个连接，连接池耗尽时阻塞等待（3秒超时），不再静默失败。

Pipeline 批量操作：多 key 多值操作合并为一次 Redis 往返（batchRpush、multiHget）。

在线用户分页：使用 Redis Sorted Set 服务端分页，万级用户分页响应 <2ms。

频道最后消息缓存：发送消息时同步写入 Redis 缓存，频道列表读取不再查 MySQL。

离线消息批量查询：拉取消息时批量查 account→userId 映射，消除 N+1 Redis 查询。

离线消息批量推送：给离线成员推送消息使用 Pipeline 一次发送，千人频道推送延迟 <5ms。

会话列表 Redis 缓存：rebuildConversations 优先读 Redis 缓存，未命中再回退 MySQL。

异步日志：后端线程定时刷盘，前端写入仅内存操作，日志对业务零影响；使用 localtime_r 保证线程安全。

空闲连接回收：timerfd 定时扫描，关闭超时未活动的连接（180 秒）。

## 🧩 项目难点与解决方案
### 全局消息广播的性能瓶颈
早期版本对所有在线用户统一广播，导致单连接发送阻塞、锁竞争严重。

解决：将消息按群组频道和私聊目标拆分，仅向目标群组/频道的在线成员或指定用户发送；ConnectionManager 按 EventLoop 分组存储用户，广播时按线程分发任务，避免全局锁。
